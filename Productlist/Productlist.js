  ...
  if (searchTerm && searchTerm.trim() !== "") {
    ...
    filteredCourses = filteredCourses.filter(
      (course) =>
        ...
        String(course.Price || "").toLowerCase().includes(term) ||
        String(course.Size || "").toLowerCase().includes(term)
    );
  }
    ...
    card.innerHTML = `
      <a href="FullpageOrder.html?id=${course.Id}" class="block relative">
        ...
        <div class="h-48 bg-[#526474] flex items-center justify-center overflow-hidden">
          <img src="${course.Icon}" alt="${course.Tittle}" class="w-full h-full object-cover" />
        </div>
        <div class="overflow-hidden px-2  ">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-1 text-sm">
              ...
              <span class="text-gray-800 font-semibold">(${course.Ratting || 0})</span>
            </div>
          </div>
          <p class="text-gray-500 text-xs mt-[-5px]  overflow-hidden h-4">${course.Specification || ""}</p>
          <div class="flex items-center justify-between  ">
            <span class="font-extrabold text-lg">₹${course.Price || 0}<span class="text-[10px] text-gray-800"> ${course.Size || ""}</span></span>
